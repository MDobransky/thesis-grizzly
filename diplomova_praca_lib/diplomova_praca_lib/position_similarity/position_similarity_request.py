...
class RegionsData:
    def __init__(self, data):
        ...
        self.unique_src_paths = {x: i for i, x in enumerate(sorted(set(self.src_paths)))}
...
def paths_ids(paths: List[str]) -> List[int]:
    global regions_env
    return [regions_env.regions_data.unique_src_paths[path] for path in paths]